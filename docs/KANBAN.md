...
**SND-103 — 本地音频扫描与元数据提取**

持久化资料库和目录授权链路已经接通。下一步扫描一个真实目录，提取
MP3/FLAC 的标题、艺人、专辑、曲序、时长、封面和内嵌歌词，并把一次重扫
作为原子事务写入现有 Repository。

## 进行中

| 编号 | 优先级 | 卡片 | 验收标准 |
| --- | --- | --- | --- |
| SND-103 | P0 | 扫描一个选定目录并提取 MP3/FLAC 元数据。 | 读取标题、艺人、专辑、曲序、时长、封面和内嵌歌词；安全忽略不支持或损坏的文件；重扫以事务方式增删改；fixture 和解析测试通过。 |
...
| 编号 | 已通过 | 待完成 |
| --- | --- | --- |
| SND-102 | Android 16 ARM64：SAF 选择 `Music` 后强制停止并重启，仍恢复为“已授权”；macOS：NSOpenPanel 选择 `Music`、保存 bookmark、退出并重启后恢复为 `available`；单元测试和两端 Debug 构建通过。 | Windows 主机执行 `flutter build windows --debug`，运行后选择一个目录、重启确认仍可用，再临时重命名目录确认显示“文件夹不可用”。当前 macOS 主机不能完成此项。 |
